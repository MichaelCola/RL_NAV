...
class Arguments:
    def __init__(self, agent=None, env=None, gpu_id=None, if_on_policy=False):
        ...
        self.env_eval = env  # the environment for evaluating
        self.gpu_id = gpu_id  # choose the GPU for running. gpu_id is None means set it automatically

        '''Arguments for training (off-policy)'''
        self.net_dim = 2 ** 8  # the network width   2**8
        self.batch_size = 2 ** 8  # num of transitions sampled from replay buffer.   2**8
        self.repeat_times = 2 ** 0  # repeatedly update network to keep critic's loss small
        self.target_step = 2 ** 10  # collect target_step, then update network  2**10
        # self.target_step = 2 ** 2  #调试用
        ...
        self.gamma = 0.90  # discount factor of future rewards  0.99
...
def train_and_evaluate(args):
    ...
    while not ((if_break_early and if_reach_goal)
        ...
        total_step += steps
        print("start update")
        obj_a, obj_c = agent.update_net(buffer, target_step, batch_size, repeat_times = repeat_times)
        print("start evel")
...
def get_episode_return(env, act, device) -> (float, int):
    ...
    for episode_step in range(max_step):
        s_tensor = torch.as_tensor((state,),dtype=torch.float32, device=device).detach_()
        a_tensor = act(s_tensor)

        if if_discrete:
            a_tensor = a_tensor.argmax(dim=1)
        action = a_tensor.detach().cpu().numpy()[0] # not need detach(), because with torch.no_grad() outside
        # print("evel_action:",action)
...
def explore_before_training(env, buffer, target_step, reward_scale, gamma) -> int:
    ...
    state = env.reset()
    steps = 0
    past_action = np.zeros(action_dim)
    ...
    while steps < target_step:
        ...
        past_action = deepcopy(action)
        # print("ranodm_acton:",action)
        # time_fin  = time.time()

        # print("time" , time_fin - time_)
