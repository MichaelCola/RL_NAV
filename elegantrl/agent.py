...
from elegantrl.net import Actor, ActorSAC, ActorPPO ,ActorDiscretePPO
...
class AgentBase:
    ...
    def explore_env(self, env, buffer, target_step, reward_scale, gamma) -> int:
        ...
        """
        #training 前 reset
        self.statestate = env.reset()
        past_action = np.zeros(2)
        for _ in range(target_step):
            action = self.select_action(self.state)
            print(action)
            next_s, reward, done, _ = env.step(action,past_action)
            past_action = deepcopy(action)
            # print("net_acton:",action)
...
class AgentDDPG(AgentBase):
    ...
    def select_action(self, state) -> np.ndarray:
        states = torch.as_tensor((state,), dtype=torch.float32, device=self.device).detach_()
        action = self.act(states)[0].cpu().detach().numpy()
        
        return (action + self.ou_noise()).clip(-1, 1)
        # return (action + self.ou_noise())
...
class AgentTD3(AgentDDPG):
    def __init__(self):
        super().__init__()
        self.explore_noise = 0.001  # standard deviation of explore noise   0.1
        self.policy_noise = 0.002  # standard deviation of policy noise   0.2
        self.update_freq = 2  # delay update frequency, for soft target update  2
...
class AgentPPO(AgentBase):
    ...
    def explore_env(self, env, buffer, target_step, reward_scale, gamma) -> int:
        ...
        while actual_step < target_step:
            ...
            for _ in range(env.max_step):
                ...
                next_state, reward, done, _ = env.step(np.tanh(action) , past_action)
                actual_step += 1
                past_action = deepcopy(np.tanh(action))
    ...
    def update_net(self, buffer, _target_step, batch_size, repeat_times=4) -> (float, float):
        ...
        with torch.no_grad():
            ...
            buf_value = torch.cat([self.cri(buf_state[i:i + bs]) for i in range(0, buf_state.size(0), bs)], dim=0)  #以一个batch的数据进行输入
        ...
        for _ in range(int(repeat_times * buf_len / batch_size)):
            ...
            obj_united.backward()
            self.optimizer.step()
        # print(self.act.a_std_log.mean().item() , self.act.a_std_log.mean(),obj_critic)
